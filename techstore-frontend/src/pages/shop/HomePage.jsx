...
const perks = [
  {
    ...
    svg: (
      <svg viewBox="0 0 40 40" className="w-8 h-8 sm:w-10 sm:h-10 flex-shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const brands = [
  { name: 'Samsung',  style: 'font-black text-lg tracking-tight text-surface-800' },
  { name: 'Nokia',    style: 'font-black text-lg tracking-wider uppercase text-surface-800' },
  { name: 'Apple',    style: 'font-semibold text-lg text-surface-800' },
  { name: 'Huawei',   style: 'font-bold text-base tracking-wide text-surface-700' },
  { name: 'Infinix',  style: 'font-bold text-lg italic text-surface-800' },
  { name: 'Tecno',    style: 'font-black text-lg tracking-widest uppercase text-surface-800' },
]
      ...
      <main className="flex-1">
        ...
        <section className="relative bg-[#0b1154] overflow-hidden">
          ...
          <div className="page-container relative">
            <div className="grid lg:grid-cols-[1fr_320px] xl:grid-cols-[1fr_360px] gap-0">
              ...
              {/* Left */}
              <div className="flex flex-col justify-center py-10 lg:py-16 pr-0 lg:pr-12 lg:border-r lg:border-white/10">
                <div className="inline-flex items-center gap-2 self-start bg-white/10 border border-white/15 rounded-full px-3 py-1.5 mb-5">
                ...
                <h1 className="font-display leading-none tracking-tight mb-4">
                  <span className="block text-[2.4rem] xs:text-5xl sm:text-6xl xl:text-7xl font-black text-white">
                    Premium tech,
                  </span>
                  <span className="block text-[2.4rem] xs:text-5xl sm:text-6xl xl:text-7xl font-black text-brand-400">
                    delivered fast.
                  </span>
                </h1>

                <p className="text-white/50 text-sm sm:text-base leading-relaxed max-w-md mb-6">
                  Nigeria's favourite online electronics store. Phones, tablets, watches and more — from brands you trust.
                </p>

                <div className="flex flex-wrap gap-3 mb-6">
                  <Link to="/products"
                    className="inline-flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-all duration-200 active:scale-95">
                    Shop Now <ArrowRight className="w-4 h-4" />
                  </Link>
                  <Link to="/products?sortBy=price&sortDir=asc"
                    className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/15 border border-white/15 text-white/90 font-medium px-5 py-2.5 rounded-xl text-sm transition-all duration-200">
                    View Deals
                  </Link>
                </div>

                {/* Stats strip — visible on mobile only (desktop sees them in the right panel) */}
                <div className="grid grid-cols-4 gap-2 mb-6 lg:hidden">
                  {stats.map(s => (
                    <div key={s.label} className="bg-white/8 border border-white/10 rounded-xl py-2.5 text-center">
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map(cat => (
                    <Link key={cat.id} to={`/products?categoryId=${cat.id}`}
                      className="flex items-center gap-2 bg-white/8 hover:bg-white/12 border border-white/10 hover:border-white/20 rounded-xl px-3 py-2 transition-all duration-200 group">
              ...
              {/* Right — Hot right now (desktop only) */}
        ...
        <section className="border-b border-surface-100 bg-white">
          <div className="page-container py-5 sm:py-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-x-3 gap-y-5 sm:gap-x-4 sm:gap-y-6">
              {perks.map((p, i) => (
                <div key={p.title}
                  className={`flex items-center gap-2 sm:gap-3 ${i > 0 ? 'lg:border-l lg:border-surface-100 lg:pl-6' : ''}`}>
                  {p.svg}
                  <div>
                    <p className="text-xs sm:text-sm font-bold text-surface-900">{p.title}</p>
                    <p className="text-[11px] sm:text-xs text-surface-500 mt-0.5 leading-tight">{p.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="border-b border-surface-100 bg-white">
          <div className="page-container py-5 sm:py-7">
            <div className="flex items-center gap-8 sm:gap-10 overflow-x-auto scrollbar-hide">
              {brands.map(b => (
                <Link key={b.name} to={`/products?query=${b.name}`}
                  ...
                  {b.name === 'Apple' && (
                    <svg viewBox="0 0 24 24" className="w-4 h-4 sm:w-5 sm:h-5 fill-surface-800" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* ── CATEGORIES ── */}
        <section className="page-container py-10 lg:py-16">
          <div className="flex items-end justify-between mb-6 sm:mb-8">
            <div>
              <p className="text-[10px] font-bold text-surface-400 uppercase tracking-[0.15em] mb-1">Browse</p>
              <h2 className="font-display text-2xl sm:text-3xl font-black text-surface-900 leading-none tracking-tight">
                Shop by<br />
            ...
            {categories.map((cat, i) => (
              <Link key={cat.id} to={`/products?categoryId=${cat.id}`}
                className={`group relative rounded-2xl overflow-hidden
                  ...
                  ${i === 0 ? 'lg:row-span-2 min-h-[180px] sm:min-h-[200px] lg:min-h-[340px]' : 'min-h-[130px] sm:min-h-[140px] lg:min-h-[160px]'}`}>
                <img src={cat.img} alt={cat.name}
                  className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-black/10" />
                <div className="absolute bottom-3 left-3 right-3 sm:bottom-4 sm:left-4 sm:right-4">
                  <p className="font-display font-bold text-white text-base sm:text-lg leading-none drop-shadow-sm">
                    {cat.name}
                  </p>
                  <p className="text-white/60 text-[11px] sm:text-xs mt-0.5 sm:mt-1">{cat.desc}</p>
                  <div className="flex items-center gap-1 mt-2 text-brand-300 text-xs font-semibold opacity-0 group-hover:opacity-100 transition-all duration-200 translate-y-1 group-hover:translate-y-0">
          ...
          <div className="page-container py-10 lg:py-12">
            <div className="flex items-end justify-between mb-6 sm:mb-8">
              <div>
                ...
                <h2 className="font-display text-2xl sm:text-3xl font-black text-surface-900 leading-none tracking-tight">
                  Featured<br />
                  <span className="text-surface-400">products.</span>
                </h2>
              </div>
              <Link to="/products"
                className="text-sm text-brand-600 hover:text-brand-700 flex items-center gap-1 font-medium">
        ...
        <section className="page-container py-8 sm:py-10">
          <div className="grid lg:grid-cols-2 gap-4">
            ...
            {/* Deals CTA */}
            <div className="relative rounded-2xl overflow-hidden min-h-[200px] sm:min-h-[220px]">
              <img
              ...
              <div className="relative p-5 sm:p-8 flex flex-col justify-between h-full">
                <div>
                  <span className="inline-block text-[10px] font-bold uppercase tracking-[0.12em] text-brand-400 mb-2 sm:mb-3">
                    Limited Time
                  </span>
                  <h3 className="font-display text-2xl sm:text-3xl font-black text-white leading-none tracking-tight">
                    Up to 40% off<br />
                    <span className="text-white/60">last season.</span>
                  </h3>
                </div>
                <Link to="/products?sortBy=price&sortDir=asc"
                  className="self-start mt-5 sm:mt-6 inline-flex items-center gap-2 bg-white text-surface-900 font-semibold px-4 sm:px-5 py-2.5 rounded-xl text-sm hover:bg-surface-100 transition-colors">
            ...
            {/* Social proof */}
            <div className="rounded-2xl border border-surface-200 bg-surface-50 p-5 sm:p-8 flex flex-col justify-between">
              <div>
                ...
                <p className="text-surface-500 text-xs mb-4 sm:mb-5">Based on 2,400+ verified reviews</p>
                <div className="space-y-3">
                  {[
        ...
        <section className="border-t border-surface-100 bg-surface-50">
          <div className="page-container py-8 sm:py-10">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
              <div>
                <h3 className="font-display text-lg sm:text-xl font-bold text-surface-900">Stay in the loop.</h3>
                <p className="text-surface-500 text-sm mt-0.5">New arrivals, deals, and tech news — weekly.</p>
              </div>
              <div className="flex gap-2 w-full sm:w-auto">
                <input type="email" placeholder="your@email.com" className="input flex-1 sm:w-60 text-sm" />
